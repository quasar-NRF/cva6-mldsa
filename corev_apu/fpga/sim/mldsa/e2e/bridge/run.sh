...
#!/bin/bash
# =============================================================================
# ML-DSA-65 End-to-End — BRIDGE simulation (accelerator + AXI bridge)
# =============================================================================
# What this tests:
#   The full KeyGen → Sign → Verify pipeline through axi_mldsa_bridge, driven
#   by an AXI4 master BFM. This is exactly what the CVA6 CPU does at runtime
#   when running the full signing protocol.
#
...
#   Between phases, the testbench writes CTRL=0x00 to clear the start bit.
#   The bridge's ctrl_start_rise is a rising-edge detector; without clearing,
#   the start bit stays high from Phase 1 and the next phase's CTRL write
#   doesn't trigger the 2-phase start sequence (RST_CYCLES + START_DELAY).
#
# Pass criterion:
...
echo "==================================================================="
echo " ML-DSA End-to-End BRIDGE sim (sec_lvl=${SEC_LVL} / ML-DSA-${KAT_SUF}, KeyGen→Sign→Verify)"
echo "==================================================================="
...
# TUMCREATE: BRG_DEBUG=1 env var enables bridge FIFO visibility (push/drop/pop logging).
BRG_DBG_FLAGS=""
if [ "${BRG_DEBUG:-0}" = "1" ]; then
    BRG_DBG_FLAGS="-d BRG_DEBUG"
    echo "  BRG_DEBUG=1 enabled (bridge FIFO visibility)" | tee -a "$LOG"
fi
R=$($XVLOG --sv --relax -d XSIM -d VERILATOR -d SEC_LVL=${SEC_LVL} $BRG_DBG_FLAGS -i "$PULP_AXI_INC" -i "$PULP_COMMON_INC" \
    -i "$COMMON" -i "$SRC" \
...
$XELAB --relax -d XSIM -d VERILATOR -d SEC_LVL=${SEC_LVL} $BRG_DBG_FLAGS "work.${TB_NAME}" -snapshot "sim_bridge_${PHASE}" 2>&1 \
...
if grep -q "testbench done - PASS" "$SIM_RUN_LOG"; then
    echo "==========================================================="
    echo " RESULT: PASS — chained KeyGen→Sign→Verify through bridge accepted the signature"
    echo "==========================================================="
    exit 0
