...
#!/bin/bash
# =============================================================================
# ML-DSA-65 Sign — BRIDGE simulation (accelerator + AXI bridge)
# =============================================================================
# What this tests:
#   The full memory-mapped peripheral path for Sign: an AXI4 master BFM drives
#   axi_mldsa_bridge, which feeds the accelerator. This is the exact path the
#   CVA6 CPU uses at runtime.
#
# Memory map: see keygen/bridge/run.sh header (same for all phases).
# Sign CTRL value: (sec_lvl=3 << 3) | (mode=2 << 1) | start=1 = 0x1D
#
...
#   1. z    : 400 words (3200B)
#   2. h    : 8 words   (61B, last padded)
#   3. ctilde : 6 words (48B)
#   Total: 414 words
#
# Pass criterion:
#   Output signature matches the NIST SigGen KAT byte-for-byte.
# =============================================================================
# Usage:
#   ./run.sh
# =============================================================================
...
if grep -q "testbench done - PASS" "$SIM_RUN_LOG"; then
    echo "==========================================================="
    echo " RESULT: PASS — Sign through bridge matches KAT byte-for-byte"
    echo "==========================================================="
    exit 0
elif grep -q "testbench done - FAIL" "$SIM_RUN_LOG"; then
    echo "==========================================================="
    echo " RESULT: FAIL — signature mismatch (see sim output)"
    echo "==========================================================="
    exit 1
